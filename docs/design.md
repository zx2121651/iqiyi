...
- **数据库**:
    ...
    - 开发环境为了便捷，使用 `SQLite`。
- **缓存**:
    - `Redis` 已被引入作为 `Celery` 和 `Channels` 的后端，同时也可以用于缓存、降低数据库压力等。
- **异步任务队列 (Celery)**:
    - 项目集成了 `Celery` 来处理耗时操作，例如视频封面生成。这使得API可以快速响应，将重量级任务移至后台工作进程（Worker）处理。
- **云存储 (S3)**:
    - 项目已配置为使用兼容S3协议的云存储。所有用户上传的媒体文件（图片、视频）都将被上传至云端，便于扩展、管理和通过CDN分发。
